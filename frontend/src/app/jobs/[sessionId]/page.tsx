...
export default function SessionDetailPage() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setReprocessingJobs(prev => {
          ...
          const docs: Job[] = Array.isArray(data?.documents) ? data.documents : []
          ...
          for (const jobId of prev) {
            ...
            if (['queued', 'pending', 'processing', 'uploaded', 'failed', 'cancelled'].includes(doc.status)) {
              next.delete(jobId)
            }
          }
          return next
  ...
  const filteredJobs = session?.documents.filter(j => {
    const matchesName = j.original_filename.toLowerCase().includes(searchQuery.toLowerCase())
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="mb-6 rounded-2xl border border-border-light bg-surface-light/85 p-5 shadow-sm backdrop-blur-sm dark:border-border-dark dark:bg-surface-dark/70">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <div>
                <h1 className="mb-2 text-3xl font-black tracking-tight">{session?.session_name}</h1>
              </div>
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              ...
              <span className="inline-flex items-center gap-1 rounded-full bg-emerald-500/10 px-2.5 py-1 text-[11px] font-bold text-emerald-600 dark:text-emerald-400">
                ...
                완료 {session?.documents.filter(doc => doc.status === 'completed').length ?? 0}개
              </span>
              <span className="inline-flex items-center gap-1 rounded-full bg-red-500/10 px-2.5 py-1 text-[11px] font-bold text-red-600 dark:text-red-400">
                ...
                실패 {session?.documents.filter(doc => doc.status === 'failed').length ?? 0}개
              </span>
            </div>
          </div>
              ...
              <tbody className="divide-y divide-border-light dark:divide-border-dark">
                {filteredJobs.length === 0 ? (
                  ...
                  filteredJobs.map(job => (
                    <tr key={job.job_id} className="transition-colors hover:bg-primary/5 dark:hover:bg-primary/10">
                        ...
                        <div className="flex flex-col items-center gap-2">
                          {(() => {
                            ...
                            const displayStatus = isReprocessing
                              ? 'queued'
                              : (job.status === 'uploaded' ? 'queued' : job.status)
        ...
        <div className="fixed inset-y-0 left-64 right-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border border-border-light bg-surface-light p-5 shadow-xl dark:border-border-dark dark:bg-surface-dark">
            <div className="mb-3 flex items-center gap-2">
              <span className="material-symbols-outlined text-orange-500">refresh</span>
              <h2 className="text-base font-black">OCR 재실행 확인</h2>
            </div>
            <p className="text-sm text-text-secondary-light dark:text-text-secondary-dark">
              <span className="font-bold text-text-primary-light dark:text-text-primary-dark">{reprocessTarget.original_filename}</span>
              {' '}파일을 다시 처리할까요?
            </p>
