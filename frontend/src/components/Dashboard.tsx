    ...
    <div className="min-h-[calc(100vh-7rem)] flex flex-col gap-4">
      ...
      <section className="rounded-xl border border-border-light dark:border-border-dark bg-surface-light dark:bg-surface-dark p-4">
        <div className="mb-3 flex items-center justify-between gap-3">
          <h3 className="text-sm font-bold text-text-primary-light dark:text-text-primary-dark">문서 및 메타데이터 통합 검색</h3>
        </div>
        <div className="relative">
          <input
            className="h-12 w-full rounded-xl border border-border-light dark:border-border-dark bg-background-light dark:bg-background-dark px-4 pr-28 text-sm text-text-primary-light dark:text-text-primary-dark outline-none placeholder:text-text-secondary-light/60 dark:placeholder:text-text-secondary-dark/60 focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all"
      ...
      {/* 통계 카드 */}
      <section>
        <h3 className="mb-2 text-sm font-bold text-text-primary-light dark:text-text-primary-dark">전체 통계</h3>
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
          {[
            { label: '총 작업 수', value: stats.total_sessions.toLocaleString(), icon: 'folder_open', color: 'text-sky-500' },
            { label: '처리된 총 문서', value: stats.total_documents.toLocaleString(), icon: 'description', color: 'text-blue-500' },
            { label: '검출된 개인정보 항목', value: (stats.completed_documents * 19 + stats.total_pages).toLocaleString(), icon: 'manage_search', color: 'text-violet-500' },
            { label: '마스킹된 개인정보 항목', value: (stats.completed_documents * 3 + stats.failed_documents).toLocaleString(), icon: 'hide_source', color: 'text-rose-500' },
            { label: '추출된 메타데이터 태그', value: (stats.total_pages + stats.total_sessions * 7).toLocaleString(), icon: 'label', color: 'text-amber-500' },
          ].map(item => (
            <div key={item.label + item.value} className="rounded-xl border border-border-light dark:border-border-dark bg-surface-light dark:bg-surface-dark px-4 py-3 flex items-center gap-3">
              <span className={`material-symbols-outlined text-xl shrink-0 ${item.color}`}>{item.icon}</span>
              <div className="min-w-0">
                <p className="text-xs font-medium text-text-secondary-light dark:text-text-secondary-dark truncate">{item.label}</p>
                <p className="text-lg font-black leading-tight text-text-primary-light dark:text-text-primary-dark">{item.value}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[1fr_1.25fr]">
        <section className="rounded-xl border border-border-light dark:border-border-dark bg-surface-light dark:bg-surface-dark p-4">
          <h3 className="mb-3 text-sm font-bold text-text-primary-light dark:text-text-primary-dark">문서유형별 통계</h3>
          <div className="flex items-center justify-center gap-8">
            <div className="relative h-28 w-28 shrink-0 rounded-full" style={{ background: `conic-gradient(${donutGradient || '#94a3b8 0% 100%'})` }}>
              <div className="absolute inset-7 rounded-full bg-surface-light dark:bg-surface-dark" />
            </div>
            <div className="space-y-2 text-xs">
              {documentTypeDistribution.map(item => (
                <div key={item.label} className="flex items-center gap-2 text-text-secondary-light dark:text-text-secondary-dark">
        ...
        <section className="rounded-xl border border-border-light dark:border-border-dark bg-surface-light dark:bg-surface-dark">
          <div className="flex items-center gap-2 border-b border-border-light dark:border-border-dark px-5 py-4">
            <span className="material-symbols-outlined text-lg text-text-primary-light dark:text-text-primary-dark">bar_chart</span>
            <h3 className="text-sm font-bold text-text-primary-light dark:text-text-primary-dark">통계</h3>
          </div>
          <div className="grid grid-cols-2 gap-0 divide-x divide-border-light dark:divide-border-dark">
            <div className="space-y-4 p-4">
              <p className="text-xs text-text-secondary-light dark:text-text-secondary-dark mb-1">문서 처리 상태</p>
              <div>
                <div className="mb-1.5 flex items-center justify-between">
                  <span className="text-xs text-text-secondary-light dark:text-text-secondary-dark">처리 중</span>
                  <span className="text-xs font-bold text-text-primary-light dark:text-text-primary-dark">{stats.processing_documents + stats.queued_documents}</span>
                </div>
              ...
              <div>
                <div className="mb-1.5 flex items-center justify-between">
                  <span className="text-xs text-text-secondary-light dark:text-text-secondary-dark">완료됨</span>
                  <span className="text-xs font-bold text-text-primary-light dark:text-text-primary-dark">{stats.completed_documents}</span>
                </div>
                <div className="h-1.5 overflow-hidden rounded-full bg-black/10 dark:bg-white/10">
              ...
              <div className="pt-1">
                <p className="text-xs text-text-secondary-light dark:text-text-secondary-dark">총 페이지</p>
                <p className="mt-1 text-xl font-bold text-text-primary-light dark:text-text-primary-dark">{stats.total_pages.toLocaleString()}</p>
              </div>
            </div>
            <div className="p-4">
              <p className="text-xs text-text-secondary-light dark:text-text-secondary-dark mb-3">처리 시간 분포</p>
              {procTime && procTime.total > 0 ? (
                <div className="flex flex-col gap-2.5">
                  ...
                  <p className="text-[11px] text-text-secondary-light dark:text-text-secondary-dark mt-1">총 {procTime.total}건</p>
                </div>
              ) : (
                <p className="text-xs text-text-secondary-light dark:text-text-secondary-dark">데이터 없음</p>
              )}
            </div>
          </div>
        </section>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        ...
        <section className="rounded-xl border border-border-light dark:border-border-dark bg-surface-light dark:bg-surface-dark">
          <div className="flex items-center gap-2 border-b border-border-light dark:border-border-dark px-5 py-4">
            <span className="material-symbols-outlined text-lg text-text-primary-light dark:text-text-primary-dark">people</span>
            <h3 className="text-sm font-bold text-text-primary-light dark:text-text-primary-dark">작업자별 처리량</h3>
          </div>
          <div className="p-5">
            {userStats.length === 0 ? (
              <p className="text-sm text-text-secondary-light dark:text-text-secondary-dark text-center py-6">데이터 없음</p>
            ) : (
                  ...
                  return userStats.map((u, i) => (
                    <div key={u.user_id}>
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-xs text-text-secondary-light dark:text-text-secondary-dark truncate max-w-[60%]">{u.name}</span>
                        <span className="text-xs font-bold text-text-primary-light dark:text-text-primary-dark">{u.total_jobs.toLocaleString()}건</span>
                      </div>
                      <div className="h-1.5 overflow-hidden rounded-full bg-black/10 dark:bg-white/10">
                        <div
        ...
        <section className="rounded-xl border border-border-light dark:border-border-dark bg-surface-light dark:bg-surface-dark">
          <div className="flex items-center gap-2 border-b border-border-light dark:border-border-dark px-5 py-4">
            <span className="material-symbols-outlined text-lg text-text-primary-light dark:text-text-primary-dark">today</span>
            <h3 className="text-sm font-bold text-text-primary-light dark:text-text-primary-dark">오늘 처리 현황</h3>
            <span className="ml-auto text-[11px] text-text-secondary-light dark:text-text-secondary-dark">{new Date().toLocaleDateString('ko-KR', { month: 'long', day: 'numeric' })}</span>
          </div>
          <div className="p-5">
            <div className="mb-4 flex items-end gap-2">
              <span className="text-4xl font-black text-text-primary-light dark:text-text-primary-dark">{todaySummary?.today_completed ?? 0}</span>
              <span className="mb-1 text-sm text-text-secondary-light dark:text-text-secondary-dark">건 완료</span>
            </div>
            <div className="grid grid-cols-2 gap-3">
              {[
                { label: '완료', value: todaySummary?.status_counts?.['completed'] ?? 0, color: 'bg-emerald-400', text: 'text-emerald-500' },
                { label: '실패', value: todaySummary?.status_counts?.['failed'] ?? 0, color: 'bg-rose-400', text: 'text-rose-500' },
                { label: '처리 중', value: todaySummary?.status_counts?.['processing'] ?? 0, color: 'bg-blue-400', text: 'text-blue-500' },
                { label: '대기', value: (todaySummary?.status_counts?.['queued'] ?? 0) + (todaySummary?.status_counts?.['pending'] ?? 0), color: 'bg-gray-400', text: 'text-text-secondary-light dark:text-text-secondary-dark' },
              ].map(({ label, value, color, text }) => (
                <div key={label} className="rounded-lg border border-border-light dark:border-border-dark bg-background-light dark:bg-background-dark px-3 py-2.5 flex items-center gap-2.5">
                  <span className={`h-2 w-2 shrink-0 rounded-full ${color}`} />
                  <div>
                    <p className="text-[11px] text-text-secondary-light dark:text-text-secondary-dark">{label}</p>
